  ...
  with tf.Graph().as_default(), tf.Session() as sess:
      ...
      for iteration in xrange(FLAGS.iterations):
          ...
          if (iteration % 2  == 0):
              ...
              saved_model_path = saver.save(sess, './my-model.ckpt')
...
def feedforward(sv_path, fpath):
  ...
  sess = tf.Session()
  saver = tf.train.Saver()
  saver.restore(sess, sv_path)
  feed_dict = {
    X_content: np.array([dataset.get_image(fpath)])
  }

  image= sess.run([transferred_image], feed_dict=feed_dict)
  image = image[0]
  image = image.reshape((256, 256, 3))
  img = np.clip(image, 0, 255).astype(np.uint8)
  scipy.misc.imsave('feed.jpg', img)

def main(_):
  if tf.gfile.Exists(FLAGS.log_dir):
    tf.gfile.DeleteRecursively(FLAGS.log_dir)
  tf.gfile.MakeDirs(FLAGS.log_dir)
  saved_model_path = run_training()
  print saved_model_path
  feedforward('./my-model.ckpt', 'test_data/baby.jpg')
